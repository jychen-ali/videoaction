...
class direct_ptr_net:
    def __init__(self, batch, hidden, keep_prob=1.0, is_train=None, scope="direct_ptr_net"):
    ...
    def __call__(self, init, match, d, mask):
        with tf.variable_scope(self.scope):
            ...
            d_init = init * self.dropout_mask
            u = tf.concat([tf.tile(tf.expand_dims(d_init, axis=1), [
                1, tf.shape(d_match)[1], 1]), d_match], axis=2) 
            s0 = tf.nn.tanh(dense(u, d, use_bias=False, scope="s0"))
...
def dot_attention(inputs, memory, mask, hidden, keep_prob=1.0, is_train=None, scope="dot_attention"):
    with tf.variable_scope(scope):
        ...
        #with tf.variable_scope("cross_gate"):
        #    dim1 = inputs.get_shape().as_list()[-1]
        #    dim2 = outputs.get_shape().as_list()[-1]
        #    gate1 = tf.nn.sigmoid(dense(inputs, dim2, use_bias=False, scope="g_inputs"))
        #    gate2 = tf.nn.sigmoid(dense(outputs, dim1, use_bias=False, scope="g_outputs"))
        #    inputs = inputs * gate2
        #    outputs = outputs * gate1
        #    res = tf.concat([inputs, outputs], axis=2)
        #    return res

def direct_attention(inputs, memory, mask, hidden, keep_prob=1.0, is_train=None, scope="direct_attention"):
    with tf.variable_scope(scope):
        d_inputs = dropout(inputs, keep_prob=keep_prob, is_train=is_train)
        d_memory = dropout(memory, keep_prob=keep_prob, is_train=is_train)
        JX = tf.shape(inputs)[1]
